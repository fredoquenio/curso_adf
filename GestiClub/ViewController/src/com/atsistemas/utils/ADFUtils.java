...
public class ADFUtils {
    ...
    public static Object getSelectedValueFromListBinding(int index, String bindingDataControl) {
        JUCtrlListBinding ju = (JUCtrlListBinding)getBindingContainer().get(bindingDataControl);
        return ju.getValueFromList(index);
    }
     ...
     * @return
     */
    public static Object getAttributeFromIteratorByIndex(String iterator, String attr, int index) {
        Object result = null;
        Row currRow = ADFUtils.findIterator(iterator).getRowAtRangeIndex(index);
    ...
    /**
     * Método para actualizar el valor de un atributo dentro de una colección
     * @param attribute
     * @param value
     * @param iteratorName
     */
    public static void setAttributeFromIterator(String attribute, Object value, String iteratorName, boolean onlyCurrentRow) {
        DCIteratorBinding iterator = ADFUtils.findIterator(iteratorName);
        if(iterator != null && !iterator.isEmpty()){
            RowSetIterator rsi = iterator.getRowSetIterator();
            Row actual = rsi.getCurrentRow();
            if(onlyCurrentRow){
                if(actual != null){
                    actual.setAttribute(attribute, value);
                }
                else{
                    throw new RuntimeException("No Current Row found for iterator " + iteratorName);    
                }
            }
            else{
                Row first = rsi.first();
    ...
    /**
     * Método para resetear las bind variables de un VO
     * @param vo
     */
    public static void clearBindVariables(ViewObjectImpl vo){
        int tam = vo.getVariableManager().getVariables().length;
        for (int i = 0; i < tam; i++) {
            vo.getVariableManager().clearValue(vo.getVariableManager().getVariables()[i]);
        }
    }
    /**
     * Método para copiar una row
     * @param source
     * @param target
     * @param skipAttrs
     */
    public static void duplicateRow(Row source, Row target, String[] skipAttrs) {

        String[] empAttrs = source.getAttributeNames();
        //Skip copying the primay key attributes or any attributes which you want to skip
        
        List skipAttrList = new ArrayList();
        if(skipAttrs != null){
            skipAttrList = Arrays.asList(skipAttrs);
        }

        //copying all attributes one by one
        for (int i = 0; i < empAttrs.length; i++) {
            String empAttrName = empAttrs[i];

            int attrIndex = target.getAttributeIndexOf(empAttrName);
            //Checking if the attribute is in the skip attribute list and the attribute is updatable
            if (!skipAttrList.contains(empAttrName) && target.isAttributeUpdateable(attrIndex)) {
                //Setting the value for the attributes
                target.setAttribute(empAttrName, source.getAttribute(empAttrName));
            }
        }
